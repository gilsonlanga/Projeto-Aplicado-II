...
public class PesquisaVigencia extends JFrame {
	...
	private JTextField textFieldDataVigencia;
	
	String dataVigencia;
	
	
	public PesquisaVigencia() {
		setTitle("Listagem dos Treinamentos por Data de Vigência");
		...
		criarTextFieldDataVigencia("Data de Vigência");
		criarBotao("Buscar", new ButtonBuscarPorDataVigenciaHandler());
	...
	private void criarTextFieldDataVigencia(String label) {
		this.criarLabel(label);
		textFieldDataVigencia = new JTextField();
		textFieldDataVigencia.setPreferredSize(new Dimension(400,40));
		panel.add(textFieldDataVigencia);
	}
	
	private class ButtonBuscarPorDataVigenciaHandler implements ActionListener {
		
		@Override
		public void actionPerformed(ActionEvent e) {
			dataVigencia = textFieldDataVigencia.getText();
			
			criarList();
			
			
			List<Treinamento> treinamentosCarregados = treinamentoController.listarVigencia(dataVigencia);
